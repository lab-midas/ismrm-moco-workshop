...
%         [b_total_coils] = GRPE_2D_nufft(b_mc,kpos{mc},1:numel(mask{mc}),dcf{mc},siz,size(dcf{mc}),csm,ncoils,adjoint);
        FT = NUFFT(kpos{mc}(:,1) + 1i*kpos{mc}(:,2), 1, 1, 0, [size(csm,1) size(csm,2)], 2);
        [b_total_coils] = nufft_sense(b_mc,kpos{mc},1:numel(mask{mc}),dcf{mc},siz,size(dcf{mc}),csm,ncoils,adjoint,1,FT);
