function Cj = nufft3_type2(xj, fk, sign, varargin)
% 
% nufft3_type2
% 
% Description
...
% Synthax
% -------
% Cj = nufft3_type2(xj, fk, -1)
% Cj = nufft3_type2(xj, fk, -1, 'double')
% Cj = nufft3_type2(xj, fk, +1, 1e-6)
%      
% Parameters
% ----------
% xj: 3D-matrix [nj, 3]
%     Location of the non-uniform samples, normalized in [-0.5, 0.5)
% fk: 3D-matrix [ms, mt, mu]
%     Uniformly spaced samples, should be complex valued. Assumes that first
%     dimension is x, second dimension is y and third dimension is z.
% sign: integer 
...
if ~(ndims(fk) == Ndims && all(size(fk) > 1))
    error('Parameter #2 should be a 3D matrix');
end
...
%% call NUFFT
Cj = nufft3d2_mex(xj(:, 1), xj(:, 2), xj(:, 3), sign, eps, fk);
