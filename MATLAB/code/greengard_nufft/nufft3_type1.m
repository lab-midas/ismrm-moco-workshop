function Fk = nufft3_type1(xj, cj, ms, sign, varargin)
% 
% nufft3_type1
% 
% Description
% -----------
% Non-uniform Fourier transform in 3D
% 
% Synthax
% -------
% Fk = nufft3_type1(xj, cj, ms, -1)
% Fk = nufft3_type1(xj, cj, ms, -1, 'double')
% Fk = nufft3_type1(xj, cj, ms, +1, 1e-6)
%      
% Parameters
% ----------
% xj: 3D-matrix [nj, 3]
%     Location of the non-uniform samples, normalized in [-0.5, 0.5)
% cj: 1D-vector [nj] or compatible 2D matrix
%     Non-uniform samples
% ms: array of integers [3]
...
% ------
% Fk: 3D-matrix [ms]
%     Fourier transform of the non-uniform samples. Results are ordered on a 
%     regular Cartesian lattice, with the first dimension being x, the second
%     dimension being y and the third dimension being z.
% 
...
%% call NUFFT
Fk = nufft3d1_mex(xj(:, 1), xj(:, 2), xj(:, 3), cj, sign, ...
	 eps, ms(1), ms(2), ms(3));
